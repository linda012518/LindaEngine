...
int ImGuiManager::Initialize()
{
	...
	ApplyStyle(Style::UNITY);
	EnableEditorLayoutSave(true);
	LoadFont("BuiltInAssets/Fonts/Opensans/OpenSans-Bold.ttf", 18.0f * 1.5f);
	LoadFont("BuiltInAssets/Fonts/Opensans/OpenSans-Regular.ttf", 18.0f * 1.5f);
	UseFont("BuiltInAssets/Fonts/Opensans/OpenSans-Regular.ttf");
...
void ImGuiManager::ApplyStyle(Style style)
{
	switch (style)
	{
	case Style::IM_CLASSIC_STYLE: ImGui::GetStyle() = UIStyle<Style::IM_CLASSIC_STYLE>::GetStyle(); break;
	case Style::IM_DARK_STYLE: ImGui::GetStyle() = UIStyle<Style::IM_DARK_STYLE>::GetStyle(); break;
	case Style::IM_LIGHT_STYLE: ImGui::GetStyle() = UIStyle<Style::IM_LIGHT_STYLE>::GetStyle(); break;
	case Style::DUNE_DARK: ImGui::GetStyle() = UIStyle<Style::DUNE_DARK>::GetStyle(); break;
	case Style::DEFAULT_DARK: ImGui::GetStyle() = UIStyle<Style::DEFAULT_DARK>::GetStyle(); break;
	case Style::EVEN_DARKER: ImGui::GetStyle() = UIStyle<Style::EVEN_DARKER>::GetStyle(); break;
	case Style::UNITY: ImGui::GetStyle() = UIStyle<Style::UNITY>::GetStyle(); break;
	}
}

bool ImGuiManager::LoadFont(const std::string& p_path, float p_fontSize)
{
	if (m_fonts.find(p_path) == m_fonts.end())
	{
		auto& io = ImGui::GetIO();
...
bool ImGuiManager::UnloadFont(const std::string& p_path)
{
	if (m_fonts.find(p_path) != m_fonts.end())
	{
		m_fonts.erase(p_path);
		return true;
	}
...
bool ImGuiManager::UseFont(const std::string& p_path)
{
	auto foundFont = m_fonts.find(p_path);
...
void ImGuiManager::UseDefaultFont()
{
	ImGui::GetIO().FontDefault = nullptr;
}

void ImGuiManager::EnableEditorLayoutSave(bool p_value)
{
	if (p_value)
...
bool ImGuiManager::IsEditorLayoutSaveEnabled() const
{
	return ImGui::GetIO().IniFilename != nullptr;
}

void ImGuiManager::SetEditorLayoutSaveFilename(const std::string& p_filename)
{
...
void ImGuiManager::SetEditorLayoutAutosaveFrequency(float p_frequency)
{
	ImGui::GetIO().IniSavingRate = p_frequency;
}

float ImGuiManager::GetEditorLayoutAutosaveFrequency(float p_frequeny)
{
	return ImGui::GetIO().IniSavingRate;
}

void ImGuiManager::EnableDocking(bool p_value)
{
...
void ImGuiManager::ResetLayout(const std::string& p_config) const
{
