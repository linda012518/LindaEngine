...
namespace LindaEditor
{
	...
	public:
		...
		*/
		...
		) : DataWidget<std::array<T, _Size>>(values), _dataType(p_dataType), min(p_min), max(p_max), speed(p_speed), label(p_label), format(p_format)
		{
			values.fill(p_value);
		}
	...
	protected:
		void _Draw_Impl() override
		{
			...
			if (ImGui::DragScalarN((label + this->_widgetID).c_str(), _dataType, values.data(), _Size, speed, &min, &max, format.c_str()))
			{
