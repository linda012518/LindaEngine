...
namespace LindaEditor
{
	...
	public:
		...
		*/
		...
		) : DataWidget<std::array<T, _Size>>(values), _dataType(p_dataType), step(p_step), fastStep(p_fastStep), label(p_label), format(p_format), selectAllOnClick(p_selectAllOnClick)
		{
	...
	protected:
		void _Draw_Impl() override
		{
			...
			bool enterPressed = ImGui::InputScalarN((label + this->_widgetID).c_str(), _dataType, values.data(), _Size, step != 0.0f ? &step : nullptr, fastStep != 0.0f ? &fastStep : nullptr, format.c_str(), flags);
	...
	public:
		...
		LindaEngine::EventCallback<std::array<T, _Size>&> ContentChangedEvent;
		LindaEngine::EventCallback<std::array<T, _Size>&> EnterPressedEvent;
