...
void UniversalRenderPipeline::Tick()
{
    Render();
}

const std::vector<Camera*> UniversalRenderPipeline::CheckCameraList()
{
    //1 查找可用阴影相机
    //2 查找可用颜色相机
    //3 相机排序
...
void UniversalRenderPipeline::SetupShaderParameters(Camera* camera)
{
...
void UniversalRenderPipeline::SetupLightListShaderParameters(Camera* camera, UniformDataGlobal::Data& block)
{
    //1 查找可用灯光
    //2 设置灯光参数到shader
    //3 设置阴影相机
...
void UniversalRenderPipeline::SetupCameraShaderParameters(Camera* camera, UniformDataGlobal::Data& block)
{
